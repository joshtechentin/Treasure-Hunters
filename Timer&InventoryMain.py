...
class Treasure(object):
    def __init__(self, name):
        ...
        self.image = pygame.image.load("treasure/" + name + ".png").convert()
...
class Terrain(object):
    def __init__(self, row, col, name, isSolid, treasure):
        ...
        self.image = pygame.image.load("terrain/" + name + ".png")
...
class Player(object):
    def __init__(self, row, col, speed, name):
        ...
        self.image = pygame.image.load(name + " Sprites" + "/" + self.orientation + " " + str(self.animationFrame) + ".png")
    ...
    def changeImage(self):
        self.image = pygame.image.load(self.name + " Sprites" + "/" + self.orientation + " " + str(self.animationFrame) + ".png")
...
def handleGameEvents():
    global condition, currentTool, toolImage, inv, inventoryTimer, toolTimer
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if event.key == moveLeftKey:
                ...
                pass
            elif event.key == showInventory:
                pass

def executeGameFrame():
    global toolTimer, inventoryTimer
    handleGameEvents()
...
title = pygame.image.load("Title Card.png")
currentName = 0
...
while True:
    while condition:
        ...
        pygame.display.update()
        CLOCK.tick(FPS)
##    total_seconds = start_time - timePassed
##    if total_seconds < 0:
##        total_seconds = 0
##    
